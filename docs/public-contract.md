...
- current worktree `Batch 61` continuation-plan slices now include the narrow `Batch 61ac`
  ...
  builtin malformed-spec and empty- or whitespace-only `eta_symbol` preflights before selected
  terminal-node validation, so stale selected decisions cannot mask the public empty-symbol diagnostic, and
  `AmfOptions` Cutkosky request/solve wrappers now reject empty or whitespace-only `eta_symbol`
  values before ordered ending planning, provider lookup, boundary attachment, or solver execution.
...
- `GenerateBuiltinCutkoskyPhaseSpaceBoundaryRequest(...)` is the first standalone reviewed phase-space boundary-request generator; it takes `(const ProblemSpec&, const std::string& eta_symbol = "eta")`, validates `ProblemSpec` first, rejects empty or whitespace-only `eta_symbol` values as an argument error, and on the supported subset returns `{variable = eta_symbol, location = "cutkosky-phase-space", strategy = ...}` without boundary values
...
- it is the phase-space analogue of the reviewed eta->infinity wrapper: it rejects empty or whitespace-only `eta_symbol` values locally before ending planning, provider lookup, boundary attachment, or solver execution, otherwise delegates ordered-list selection through `PlanAmfOptionsEndingScheme(...)` and then the planned-decision helper, copies `request_template`, overwrites only `boundary_requests` with that singleton request, attaches through `AttachBoundaryConditionsFromProvider(...)`, and then calls the supplied `SeriesSolver` exactly once
...
- the pure helper and full-spec transform still do not touch eta planning, the retained preparation/execution helper pair still does not add generated-`x` preparation, and automatic wrapper routing remains limited to the unique explicit-linear helper selector, the already-planned single explicit-linear `AmfOptions` decision route with no complex numeric substitutions, and the builtin `Propagator` planner's reviewed unique lightlike-linear override; it does not add broader cache behavior beyond the reviewed direct lightlike identity, multi-external bilinear grammar beyond unused spectator declarations, grouped-common recursion beyond the reviewed five nested layers, loop/external denominators, other builtin selector widening, or otherwise more general AMFlow-faithful linear-driver parity
