...
- current worktree `Batch 61` continuation-plan slices now include the narrow `Batch 61ac`
  ...
  builtin malformed-spec and empty, whitespace-only, or outer-whitespace-padded `eta_symbol`
  ...
  whitespace-only, or outer-whitespace-padded `eta_symbol` values before ordered ending planning,
  provider lookup, boundary attachment, or solver execution.
...
- matching overloads take a provider registry or no provider; the registry overload attaches through `AttachBoundaryConditionsFromProviderRegistry(...)`, while the no-provider overload delegates through `MakeDeferredEtaInfinityBoundaryProviderRegistry()` and therefore currently fails closed with typed `boundary_unsolved` before solver execution
- `GenerateBuiltinCutkoskyPhaseSpaceBoundaryRequest(...)` is the first standalone reviewed phase-space boundary-request generator; it takes `(const ProblemSpec&, const std::string& eta_symbol = "eta")`, validates `ProblemSpec` first, rejects empty, whitespace-only, or outer-whitespace-padded `eta_symbol` values as an argument error, and on the supported subset returns `{variable = eta_symbol, location = "cutkosky-phase-space", strategy = ...}` without boundary values
...
- that overload is the matching eta->infinity provider-registry wrapper: it preserves the same ordered ending-selection and request-copying behavior, then attaches through `AttachBoundaryConditionsFromProviderRegistry(...)` before calling the supplied `SeriesSolver` exactly once
...
- it is the phase-space analogue of the reviewed eta->infinity wrapper: it rejects empty, whitespace-only, or outer-whitespace-padded `eta_symbol` values locally before ending planning, provider lookup, boundary attachment, or solver execution, otherwise delegates ordered-list selection through `PlanAmfOptionsEndingScheme(...)` and then the planned-decision helper, copies `request_template`, overwrites only `boundary_requests` with that singleton request, attaches through `AttachBoundaryConditionsFromProvider(...)`, and then calls the supplied `SeriesSolver` exactly once
