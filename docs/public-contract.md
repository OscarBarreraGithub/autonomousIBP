...
- current worktree `Batch 61a` through `Batch 64ai` are still narrow: explicit complex kinematics
  ...
  off-path singular ledgers only when every ledger expression matches, or exact evaluated ledger
  value matches, a system-declared singular point and every recorded branch winding is zero, and
  still rejects actual contour detours, stale or value-mismatched ledgers, nonzero branch
  windings, or complex endpoints, while the
