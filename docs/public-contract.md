...
- Batch 45 and Batch 46 still do not add builtin or registered boundary providers, eta-to-infinity boundary value computation, `BoundaryCondition` generation, `SolveRequest` construction, solver execution, or manual-versus-provider equivalence. The current reviewed runtime now does generate one extra builtin phase-space terminal node `<family>::cutkosky-phase-space` through `Cutkosky`, but eta->infinity boundary attachment still only accepts the exact singleton infinity node and therefore rejects that reviewed phase-space ending explicitly
