...
- that overload is the already-planned decision helper: it performs the same singleton eta->infinity terminal-node validation after outer-whitespace trimming without resolving or planning another ending scheme, then returns the same reviewed Batch 45 request shape
