...
- that overload is the first reviewed phase-space provider-registry wrapper: it preserves the same planned-decision request-copying behavior, then attaches through `AttachBoundaryConditionsFromProviderRegistry(...)` before calling the supplied `SeriesSolver` exactly once
