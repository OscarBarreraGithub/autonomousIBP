...
- it consumes an already selected phase-space ending decision, generates the reviewed singleton Cutkosky boundary request, copies `request_template`, overwrites only `boundary_requests`, and preserves the same provider, provider-registry, or deferred-registry attachment behavior as the `AmfOptions` overloads without re-reading `amf_options.ending_schemes`
