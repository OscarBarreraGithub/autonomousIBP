...
- it is a thin single-name composition wrapper: it calls `PlanEndingScheme(...)` exactly once, requires the selected `EndingDecision::terminal_strategy` to be non-empty and already free of leading/trailing whitespace, accepts only the exact singleton supported terminal-node list `{<family>::eta->infinity}` after outer-whitespace trimming, and then delegates into `GenerateBuiltinEtaInfinityBoundaryRequest(...)`
...
- the single-name and already-planned eta->infinity request helpers reject an empty or whitespace-only `eta_symbol` before user-defined planning or terminal-node interpretation; ordinary ending resolution and planning failures otherwise preserve the existing `PlanEndingScheme(...)` diagnostics unchanged
