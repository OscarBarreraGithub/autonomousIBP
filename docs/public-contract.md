...
- `AssessInvariantGeneratedPhysicalKinematicsSegmentForBatch62(...)` is the first continuation-segment consumer on top of that point seam: it acts only when invariant-generated `start_location` and `target_location` are explicit real `s=...` assignments on the reviewed subset, reuses the exact `ProblemSpec` bindings for `t` and `msq`, and upgrades the assessment to `SingularSurface` when the closed real `s` segment crosses the reviewed threshold or the reviewed endpoint surface
- `SolveInvariantGeneratedSeries(...)` and `SolveInvariantGeneratedSeriesList(...)` now call those guardrails before invariant-generated DE construction and return typed `physical_kinematics_not_supported` or `physical_kinematics_singular` diagnostics; the singular summary now covers both singular physical points and reviewed continuation-segment crossings
- same-side reviewed interior `s` segments remain pass-through to the next invariant-generated validation layer instead of being rejected at the physical-kinematics preflight
- this seam still does not widen eta-generated segment checking, non-explicit continuation formats, reviewed `t` or `msq` segment crossings, conservative near-singular guard bands, or any generic Landau/singularity analysis outside the frozen repo-local K0 subset
