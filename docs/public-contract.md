...
- the actual SSH remote `main` state for this packet now runs through
  ...
  not-yet-accepted public/runtime seams described below through `Batch 64br`, all of which stay
...
- builtin `Cutkosky` ending planning also consumes the reviewed cut-topology loop-support, connected-component, top-sector activation, component target-support, and raw-prescription-vocabulary preflights before terminal-node emission: a cut propagator with no declared loop-momentum support, a cut surface split across disconnected declared loop-momentum components, a reviewed single declared top-level sector that omits a cut propagator, a reviewed multi-top-sector set where any cut propagator is inactive in every declared sector or where the connected cut component has no common declared active sector, any declared target integral that does not keep each connected cut component at positive support with component target-label telemetry in the diagnostic, or any standard/cut propagator carrying a raw prescription outside `-1`, `0`, or `1`, fails planning locally instead of producing `<family>::cutkosky-phase-space>`
...
- standard planning failures from `EndingScheme::Plan(...)` are treated as ordered fallback misses until the caller-supplied list exhausts, except the reviewed Cutkosky raw-prescription vocabulary, malformed cut-topology, per-cut or per-component top-sector activation, and component target-support failures are malformed-input fail-fast and stop ordered fallback before a later placeholder ending can be selected
- this batch does not yet couple ending decisions into boundary providers, `DESystem`, solver execution, or CLI behavior
