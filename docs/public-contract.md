...
- `SolveAmfOptionsEndingSchemeEtaInfinitySeries(...)`: the first narrow `AmfOptions::ending_schemes`-fed eta->infinity boundary attach-and-solve wrapper over the reviewed singleton eta->infinity request-generation seam, caller-supplied `BoundaryProvider`, and injected `SeriesSolver`
...
- it is a thin attach-and-solve wrapper over existing seams only: it reads only `amf_options.ending_schemes`, iterates that ordered list through `GeneratePlannedEtaInfinityBoundaryRequest(...)` until one singleton `<family>::eta->infinity` request succeeds, copies `request_template`, overwrites only `boundary_requests` with that singleton request, attaches through `AttachBoundaryConditionsFromProvider(...)`, and then calls the supplied `SeriesSolver` exactly once
- ordered fallback keeps the reviewed `PlanEndingSchemeList(...)` planning semantics at this wrapper boundary: empty ending lists still fail locally; ending-scheme resolver and registry `invalid_argument` failures still stop immediately; any planning-time `std::exception` from `EndingScheme::Plan(...)` falls through in caller order until the list exhausts and then preserves the final planning diagnostic unchanged; post-planning singleton-node and builtin-request generation failures preserve their existing single-name generator diagnostics; and provider attachment diagnostics propagate unchanged
- non-`ending_schemes` `AmfOptions` fields remain inert at this seam, and this wrapper does not widen into builtin provider registries, standalone `AmfOptions` boundary-generation helpers, direct `SolveDifferentialEquation(...)` provider consultation, CLI behavior, or broader ending semantics beyond the exact singleton `<family>::eta->infinity` request
