...
- the supported Batch 45/B65f subset is intentionally narrow: every propagator must be `Standard` and must have one of the exact zero mass literals `"0"`, `"+0"`, or `"-0"` after outer-whitespace trimming; the generator does not canonicalize caller-owned mass literals, well-formed specs outside that subset fail as typed `boundary_unsolved`, and malformed specs continue to fail as ordinary `invalid_argument` validation errors
