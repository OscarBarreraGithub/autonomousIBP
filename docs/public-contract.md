...
- it is the phase-space analogue of the reviewed single-name infinity helper: the name-based overload calls `PlanEndingScheme(...)` exactly once, then both overloads require the selected `EndingDecision::terminal_strategy` to be non-empty after outer-whitespace trimming, accept only the exact singleton supported terminal-node list `{<family>::cutkosky-phase-space}` after outer-whitespace trimming, and delegate into `GenerateBuiltinCutkoskyPhaseSpaceBoundaryRequest(...)`
