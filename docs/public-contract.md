...
- current worktree now also carries the first executable M7 helper on top of that scaffold only:
  ...
  phase-0 correct-digit and failure-code blockers explicitly, plus one diagnostic-review summary
  sidecar so typed-failure review blockers remain visible to M7. This remains release-prep
