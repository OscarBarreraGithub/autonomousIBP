...
- Batch 45 and Batch 46 still do not add builtin or registered boundary providers, eta-to-infinity or phase-space boundary value computation, automatic `BoundaryCondition` generation, `SolveRequest` construction, solver execution, or manual-versus-provider equivalence. The current reviewed runtime now also generates one reviewed Cutkosky phase-space request shape, but no phase-space-specific provider registry or attach-and-solve wrapper is live yet
