...
- current worktree `Batch 61` continuation-plan slices now include the narrow `Batch 61an`
  ...
  or exactly two, three, four, five, or six ordered upper-half-plane complex waypoints between real endpoints only
  ...
  seven-plus-waypoint or branch-changing contour detours, stale contour fingerprints, duplicate direct-real contour endpoints,
  ...
  and single-top-sector activation check: every cut propagator must mention at least one
  ...
  declared every cut propagator must be active in that mask before provider routing. This still
  does not widen builtin `Prescription` selection, multi-top-sector analysis, broader Cutkosky
...
- `FamilyDefinition.loop_prescriptions` plus
  ...
  `Prescription`, multi-top-sector analysis, broader Cutkosky topology analysis, and automatic
...
- builtin `Cutkosky` ending planning also consumes the reviewed cut-topology loop-support, connected-component, single-top-sector activation, and raw-prescription-vocabulary preflights before terminal-node emission: a cut propagator with no declared loop-momentum support, a cut surface split across disconnected declared loop-momentum components, a reviewed single declared top-level sector that omits a cut propagator, or any standard/cut propagator carrying a raw prescription outside `-1`, `0`, or `1`, fails planning locally instead of producing `<family>::cutkosky-phase-space>`
...
- standard planning failures from `EndingScheme::Plan(...)` are treated as ordered fallback misses until the caller-supplied list exhausts, except the reviewed Cutkosky raw-prescription vocabulary, malformed cut-topology, and single-top-sector cut-activation failures are malformed-input fail-fast and stop ordered fallback before a later placeholder ending can be selected
- this batch does not yet couple ending decisions into boundary providers, `DESystem`, solver execution, or CLI behavior
...
- the pure helper and full-spec transform still do not touch eta planning, the retained preparation/execution helper pair still does not add generated-`x` preparation, and automatic wrapper routing remains limited to the unique explicit-linear helper selector, the already-planned single explicit-linear `AmfOptions` decision route with no complex numeric substitutions, and the builtin `Propagator` planner's reviewed unique lightlike-linear override; it does not add broader cache behavior beyond the reviewed direct lightlike identity, multi-external bilinear grammar beyond unused spectator declarations, grouped-common recursion beyond the reviewed fourteen nested layers, loop/external denominators, other builtin selector widening, or otherwise more general AMFlow-faithful linear-driver parity
