...
- current worktree `Batch 61` continuation-plan slices now include the narrow `Batch 61at`
  ...
  declared target integral must keep each cut propagator at positive support before provider
  routing. This still does not
...
- `CutkoskyPhaseSpaceCutSupport`, `CutkoskyPhaseSpaceCutComponent`,
  ...
  top-sector activation, and connected-component common-sector telemetry before provider strategy
  routing, while target-support checks stay on the current `ProblemSpec` target list
