...
- current worktree `Batch 61a` through `Batch 64ai` are still narrow: explicit complex kinematics
  ...
  exactly to a real point off the direct real segment, and evaluated singular ledgers only when
  ...
  ledgerless on-segment or non-real singular declarations, nonzero branch windings, singular starts
  or interior on-path ledgers, or complex endpoints, while the
...
- on that same exact-point recognizer, open-region bindings inside the frozen conservative exact guard band `msq/4` of the reviewed pair-production threshold now return `NearSingularSurface` with a typed threshold detail before generated DE construction; endpoint-near point checks, dynamic margins, and broader Landau/singularity analysis remain deferred
...
- builtin mode `Propagator` remains a narrow bootstrap selector on the current reviewed subset only: by default it selects all non-auxiliary propagators in declaration order, preserves `mode_name == "Propagator"`, carries matching informational propagator-expression copies, and fails deterministically when that structural selection is empty. On the reviewed no-complex one-external lightlike-linear subset, exactly one explicit `kind: "linear"` / `variant: "linear"` propagator that validates through `BuildReviewedLightlikeLinearAuxiliaryPropagator(...)` is selected by itself so the existing generated-`x` lightlike solver route is used; unsupported, absent, or ambiguous explicit-linear surfaces fall back to the structural selector. On the reviewed eta-generated path the structural selector still reuses the widened `ApplyEtaInsertion(...)` transform, including the same selected-propagator outer-whitespace trim on emitted mass literals and no broader mass canonicalization claim
