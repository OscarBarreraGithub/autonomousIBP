...
- current worktree `Batch 61a` through `Batch 64ah` are still narrow: explicit complex kinematics
...
- the pure helper and full-spec transform still do not touch eta planning, the retained preparation/execution helper pair still does not add generated-`x` preparation, and automatic wrapper routing remains limited to the unique explicit-linear helper selector plus the already-planned single explicit-linear `AmfOptions` decision route with no complex numeric substitutions; it does not add broader cache behavior beyond the reviewed direct lightlike identity, broader multi-external grammar, loop/external denominators, builtin selector widening, or otherwise more general AMFlow-faithful linear-driver parity
