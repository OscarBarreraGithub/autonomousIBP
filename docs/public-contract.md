...
- current worktree `Batch 61` continuation-plan slices now include the narrow `Batch 61ap`
  ...
  and top-sector activation check: every cut propagator must mention at least one
  ...
  provider routing. This still does not widen builtin `Prescription` selection, target-sector
  analysis, broader Cutkosky
...
- `FamilyDefinition.loop_prescriptions` plus
  ...
  `Prescription`, target-sector analysis, broader Cutkosky topology analysis, and automatic
...
- builtin `Cutkosky` ending planning also consumes the reviewed cut-topology loop-support, connected-component, top-sector activation, and raw-prescription-vocabulary preflights before terminal-node emission: a cut propagator with no declared loop-momentum support, a cut surface split across disconnected declared loop-momentum components, a reviewed single declared top-level sector that omits a cut propagator, a reviewed multi-top-sector set where any cut propagator is inactive in every declared sector, or any standard/cut propagator carrying a raw prescription outside `-1`, `0`, or `1`, fails planning locally instead of producing `<family>::cutkosky-phase-space>`
...
- standard planning failures from `EndingScheme::Plan(...)` are treated as ordered fallback misses until the caller-supplied list exhausts, except the reviewed Cutkosky raw-prescription vocabulary, malformed cut-topology, and top-sector cut-activation failures are malformed-input fail-fast and stop ordered fallback before a later placeholder ending can be selected
- this batch does not yet couple ending decisions into boundary providers, `DESystem`, solver execution, or CLI behavior
...
- the reviewed phase-space request/attach wrappers now consult `family.loop_prescriptions` or, when absent, uniform raw cut prescriptions only for builtin provider-strategy refinement on cut propagators after the reviewed loop-support/connected-component preflight, while broader topological cut analysis and automatic boundary-value generation remain deferred
