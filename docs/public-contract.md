...
- it is the phase-space analogue of the reviewed ordered-list infinity helper: it reads only `amf_options.ending_schemes`, delegates ordered fallback exactly once through `PlanAmfOptionsEndingScheme(...)`, and then validates only that selected `EndingDecision` against the reviewed non-empty terminal-strategy plus singleton phase-space-node subset after outer-whitespace trimming before delegating into `GenerateBuiltinCutkoskyPhaseSpaceBoundaryRequest(...)`
...
- it is the phase-space analogue of the reviewed eta->infinity wrapper: it delegates ordered-list selection through `PlanAmfOptionsEndingScheme(...)` and then the planned-decision helper, copies `request_template`, overwrites only `boundary_requests` with that singleton request, attaches through `AttachBoundaryConditionsFromProvider(...)`, and then calls the supplied `SeriesSolver` exactly once
