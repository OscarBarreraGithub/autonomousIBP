...
- `GeneratePlannedEtaInfinityBoundaryRequest(...)`: single-name ending-planned wrapper that accepts only the exact singleton `<family>::eta->infinity` terminal-node decision after outer-whitespace trimming and then returns the reviewed builtin `eta -> infinity` `BoundaryRequest`
...
- it is a thin single-name composition wrapper: it calls `PlanEndingScheme(...)` exactly once, accepts only the exact singleton supported terminal-node list `{<family>::eta->infinity}` after outer-whitespace trimming, and then delegates into `GenerateBuiltinEtaInfinityBoundaryRequest(...)`
