...
- the actual SSH remote `main` state for this packet now runs through
  ...
  not-yet-accepted public/runtime seams described below through `Batch 64o`, all of which stay
  ...
  the current worktree also carries the local-only narrow `Batch 64a` through `Batch 64o`
...
- current worktree `Batch 61a` through `Batch 64o` are still narrow: explicit complex kinematics
  ...
  `ProblemSpec.targets` surface, and the first
  ...
  non-invariant linear solver behavior, any generated-`x` reduction-target, DE-assembly, or
...
- the pure helper and full-spec transform still do not touch eta planning, and the retained preparation/execution helpers still do not add explicit-target preparation, DE assembly, solver/wrapper behavior, broader multi-external grammar, loop/external denominators, or otherwise more general AMFlow-faithful linear-driver parity
