...
- current worktree `Batch 61a` through `Batch 64o` are still narrow: explicit complex kinematics
  ...
  survives; without loop metadata, uniform non-default raw cut prescriptions can select the
  reviewed `+i0` or no-prescription provider strategies while default raw `-i0` cuts keep the
  legacy strategy. The same reviewed strategy vocabulary is now available through a deferred builtin
...
- `FamilyDefinition.loop_prescriptions` plus
  ...
  `Propagator::prescription`; when loop metadata is absent, uniform non-default raw cut
  prescriptions may select the reviewed `+i0` or no-prescription strategies, while default raw
  `-i0` cuts keep the legacy strategy and mixed raw cut strategies fail closed. Broader builtin
...
- the reviewed phase-space request/attach wrappers now consult `family.loop_prescriptions` or, when absent, uniform non-default raw cut prescriptions only for builtin provider-strategy refinement on cut propagators, while topological cut analysis and automatic boundary-value generation remain deferred
