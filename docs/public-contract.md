...
- `GenerateBuiltinEtaInfinityBoundaryRequest(...)` is a pure library-only generator that takes `(const ProblemSpec&, const std::string& eta_symbol = "eta")`, validates `ProblemSpec` first, rejects empty or whitespace-only `eta_symbol` values as an argument error, and on the supported subset returns exactly `{variable = eta_symbol, location = "infinity", strategy = "builtin::eta->infinity"}`
