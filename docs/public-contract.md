...
- the actual SSH remote `main` state for this packet now runs through
  ...
  not-yet-accepted public/runtime seams described below through `Batch 64m`; latest head commit
  ...
  the current worktree also carries the local-only narrow `Batch 64a` through `Batch 64m`
...
- current worktree `Batch 61a` through `Batch 64m` are still narrow: explicit complex kinematics
...
- the pure helper and full-spec transform still do not touch eta planning, and the new preparation/execution helpers still do not add explicit-target preparation, generated-`x` reduction targets, DE assembly, solver/wrapper behavior, broader multi-external grammar, loop/external denominators, or otherwise more general AMFlow-faithful linear-driver parity
