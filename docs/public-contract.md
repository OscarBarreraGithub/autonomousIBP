...
- the actual SSH remote `main` state for this packet now runs through
  ...
  not-yet-accepted public/runtime seams described below through `Batch 64bn`, all of which stay
  ...
  the current worktree also carries the local-only narrow `Batch 64a` through `Batch 64bn`
...
- the pure helper and full-spec transform still do not touch eta planning, the retained preparation/execution helper pair still does not add generated-`x` preparation, and automatic wrapper routing remains limited to the non-`complex_mode` unique validating explicit-linear helper selector, the already-planned single explicit-linear `AmfOptions` decision route with no complex numeric substitutions, and the builtin `Propagator` planner's reviewed non-`complex_mode` unique lightlike-linear override; it does not add broader cache behavior beyond the reviewed direct lightlike identity, multi-external bilinear grammar beyond unused spectator declarations, grouped-common recursion beyond the reviewed sixteen nested layers, loop/external denominators, other builtin selector widening, or otherwise more general AMFlow-faithful linear-driver parity
