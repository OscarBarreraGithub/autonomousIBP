...
- the actual SSH remote `main` state for this packet now runs through
  ...
  not-yet-accepted public/runtime seams described below through `Batch 64aa`, all of which stay
  ...
  the current worktree also carries the local-only narrow `Batch 64a` through `Batch 64aa`
...
- the repo now also carries narrow M6 harness helpers only:
  ...
  digit-threshold and required-failure-code profiles to match across the prerequisite reports, and
  writes one blocked/pass verdict over the reviewed phase-0 packet set only while preserving those
  profile maps for later M6 composition.
  ...
  per-benchmark digit/failure-code profiles, and case-study-family digit/failure-code profiles,
  and does not launch runtime work or claim `Milestone M7` / release readiness.
...
- current worktree `Batch 61a` through `Batch 64aj` are still narrow: explicit complex kinematics
...
- non-`ending_schemes` `AmfOptions` fields remain inert at this seam, and these wrappers do not widen into builtin eta-infinity provider registries, direct `SolveDifferentialEquation(...)` provider consultation, CLI behavior, or broader ending semantics beyond the exact singleton `<family>::eta->infinity` request
