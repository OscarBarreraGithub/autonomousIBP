...
- on success it returns the exact reviewed Batch 45 request shape `{variable = eta_symbol, location = "infinity", strategy = "builtin::eta->infinity"}`; malformed specs remain ordinary `invalid_argument` before selected-terminal-node checks, selected-terminal-node mismatch and reviewed builtin-request subset failures remain typed `boundary_unsolved`, and builtin-request `invalid_argument` such as empty `eta_symbol` still stop immediately
