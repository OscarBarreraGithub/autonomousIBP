...
- `GeneratePlannedEtaInfinityBoundaryRequest(...)`: single-name ending-planned wrapper that requires selected `terminal_strategy` metadata to be non-empty after outer-whitespace trimming, accepts only the exact singleton `<family>::eta->infinity` terminal-node decision after outer-whitespace trimming, and then returns the reviewed builtin `eta -> infinity` `BoundaryRequest`
...
- it is a thin single-name composition wrapper: it calls `PlanEndingScheme(...)` exactly once, requires the selected `EndingDecision::terminal_strategy` to be non-empty after outer-whitespace trimming, accepts only the exact singleton supported terminal-node list `{<family>::eta->infinity}` after outer-whitespace trimming, and then delegates into `GenerateBuiltinEtaInfinityBoundaryRequest(...)`
...
- on the planned and `AmfOptions` eta->infinity request and solve wrappers, an empty or whitespace-only selected `terminal_strategy` now fails closed as typed `boundary_unsolved` after the selected ending is planned but before provider lookup, boundary attachment, or solver execution; this is a selected-decision metadata guard and does not require the strategy to equal any builtin name or boundary-provider strategy
