...
- `AssessInvariantGeneratedPhysicalKinematicsSegmentForBatch62(...)` is the first continuation-segment consumer on top of that point seam: it acts only when invariant-generated `start_location` and `target_location` are explicit real `s=...` assignments on the reviewed subset, reuses the exact `ProblemSpec` bindings for `t` and `msq`, upgrades the assessment to `SingularSurface` when the closed real `s` segment crosses the reviewed threshold or the reviewed endpoint surface, and now also upgrades the assessment to `NearSingularSurface` when the closed real `s` segment stays within the frozen conservative exact guard band `msq/4` of either reviewed singular `s` locus
- `SolveInvariantGeneratedSeries(...)` and `SolveInvariantGeneratedSeriesList(...)` now call those guardrails before invariant-generated DE construction and return typed `physical_kinematics_not_supported`, `physical_kinematics_singular`, or `physical_kinematics_near_singular` diagnostics; the singular summary still covers both singular physical points and reviewed continuation-segment crossings, while the new near-singular summary is limited to the reviewed exact `msq/4` segment margin only
...
- this seam still does not widen eta-generated segment checking, non-explicit continuation formats, reviewed `t` or `msq` segment crossings, full reviewed open-region certification for arbitrary explicit `s` segments, or any generic Landau/singularity analysis outside the frozen repo-local K0 subset
