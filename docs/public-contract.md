...
- that overload is the already-planned decision helper: it first preserves the reviewed `ProblemSpec` and empty-`eta_symbol` validation diagnostics as ordinary `invalid_argument`, then requires the selected `EndingDecision::terminal_strategy` metadata to be non-empty and already free of leading/trailing whitespace, performs the same singleton eta->infinity terminal-node validation after outer-whitespace trimming without resolving or planning another ending scheme, and returns the same reviewed Batch 45 request shape
- the single-name and already-planned eta->infinity request helpers reject an empty `eta_symbol` before user-defined planning or terminal-node interpretation; ordinary ending resolution and planning failures otherwise preserve the existing `PlanEndingScheme(...)` diagnostics unchanged
