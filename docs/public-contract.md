...
- non-`ending_schemes` `AmfOptions` fields remain inert at this seam, and this wrapper does not widen into builtin eta-infinity provider registries, direct `SolveDifferentialEquation(...)` provider consultation, CLI behavior, or broader ending semantics beyond the exact singleton `<family>::eta->infinity` request
