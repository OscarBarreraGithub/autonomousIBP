...
- `GeneratePlannedEtaInfinityBoundaryRequest(...)`: single-name ending-planned wrapper that requires selected `terminal_strategy` metadata to be non-empty and already free of leading/trailing whitespace, accepts only the exact singleton `<family>::eta->infinity` terminal-node decision after outer-whitespace trimming, and then returns the reviewed builtin `eta -> infinity` `BoundaryRequest`
...
- on the planned and `AmfOptions` eta->infinity request and solve wrappers, an empty, whitespace-only, or leading/trailing-whitespace selected `terminal_strategy` now fails closed as typed `boundary_unsolved` after the selected ending is planned but before provider lookup, boundary attachment, or solver execution; this is a selected-decision metadata guard and does not require the strategy to equal any builtin name or boundary-provider strategy
