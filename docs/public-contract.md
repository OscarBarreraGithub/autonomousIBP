...
- the pure helper and full-spec transform still do not touch eta planning, the retained preparation/execution helper pair still does not add generated-`x` preparation, and automatic wrapper routing remains limited to the unique explicit-linear helper selector, the already-planned single explicit-linear `AmfOptions` decision route with no complex numeric substitutions, and the builtin `Propagator` planner's reviewed unique lightlike-linear override; it does not add broader cache behavior beyond the reviewed direct lightlike identity, multi-external bilinear grammar beyond unused spectator declarations, grouped-common recursion beyond the reviewed eight nested layers, loop/external denominators, other builtin selector widening, or otherwise more general AMFlow-faithful linear-driver parity
