...
- current worktree `Batch 61` continuation-plan slices now include the narrow `Batch 61aa`
  ...
  validation, so stale selected decisions cannot mask the public empty-symbol diagnostic, and
  `AmfOptions` Cutkosky request/solve wrappers now reject empty `eta_symbol` before ordered ending
  planning, provider lookup, boundary attachment, or solver execution. The same boundary-request
  preflight now performs one narrow cut-topology support/connectivity check: every cut propagator
...
- it is the phase-space analogue of the reviewed single-name infinity helper: the name-based overload rejects empty `eta_symbol` before ending resolution or planning, calls `PlanEndingScheme(...)` exactly once when that preflight passes, then both overloads require the selected `EndingDecision::terminal_strategy` to be non-empty after outer-whitespace trimming, accept only the exact singleton supported terminal-node list `{<family>::cutkosky-phase-space}` after outer-whitespace trimming, and delegate into `GenerateBuiltinCutkoskyPhaseSpaceBoundaryRequest(...)`
...
- it is the phase-space analogue of the reviewed eta->infinity wrapper: it rejects empty `eta_symbol` locally before ending planning, provider lookup, boundary attachment, or solver execution, otherwise delegates ordered-list selection through `PlanAmfOptionsEndingScheme(...)` and then the planned-decision helper, copies `request_template`, overwrites only `boundary_requests` with that singleton request, attaches through `AttachBoundaryConditionsFromProvider(...)`, and then calls the supplied `SeriesSolver` exactly once
...
- the pure helper and full-spec transform still do not touch eta planning, the retained preparation/execution helper pair still does not add generated-`x` preparation, and automatic wrapper routing remains limited to the unique explicit-linear helper selector, the already-planned single explicit-linear `AmfOptions` decision route with no complex numeric substitutions, and the builtin `Propagator` planner's reviewed unique lightlike-linear override; it does not add broader cache behavior beyond the reviewed direct lightlike identity, multi-external bilinear grammar beyond unused spectator declarations, grouped-common recursion beyond the reviewed three nested layers, loop/external denominators, other builtin selector widening, or otherwise more general AMFlow-faithful linear-driver parity
