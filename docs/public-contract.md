...
- current worktree `Batch 61a` through `Batch 64o` are still narrow: explicit complex kinematics
  ...
  automatic boundary values remain unavailable. This still does not widen builtin `Prescription`
  selection, top-sector or Cutkosky topology analysis, or automatic phase-space boundary-value
  generation;
