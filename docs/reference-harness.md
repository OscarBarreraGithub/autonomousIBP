...
- The first retained phase-0 packet-set qualification verdict is
  ...
  ids stay synchronized, fail-closes unless their per-benchmark digit-threshold and
  required-failure-code profiles agree, and writes one blocked/pass verdict over the reviewed
  phase-0 packet set only with those profile maps preserved, but it still does not compare
