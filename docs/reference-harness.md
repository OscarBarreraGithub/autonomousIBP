...
- `tools/reference-harness/scripts/extract_amflow_solve_series_state.py`: retained AMFlow solve-series state extractor for cache directories such as `automatic_loop/primary/cache/box1_amflow/1`. It records the actual master basis, `eta` coefficient matrix, AMFlow eta-infinity boundary files, epsilon samples, and Kira target/reduction context into JSON while retaining the state-capture-time note that the direct C++ solver could not yet ingest that physical boundary state. `amflow-cli solve-series` can now load this JSON shape, evaluate the retained leading eta-infinity subsystem-sample boundary coefficients, and apply retained target reduction to those boundary coefficients; this is still boundary/comparator evidence only, not singular `eta -> 0` continuation or a phase-0 parity claim.
