...
- `M0b` is now accepted on the required phase-0 benchmark set only: the retained root above has
  ...
  `complex_kinematics -> b61n`, `feynman_prescription -> b63n`,
  `automatic_phasespace -> b63n`, and `linear_propagator -> b64ag`; it also carries
  ...
  as a matrix-only case-study row with numeric evidence rather than a current `b62p` runtime blocker.
...
- `tools/reference-harness/scripts/bootstrap_reference_harness.py --self-check`: local regression
  ...
  `b61n` / `b63n` / `b64ag` phase-0 surfaces, while the singular case-study row remains
  numeric evidence rather than a current `b62p` runtime blocker.
...
- `tools/reference-harness/scripts/qualify_milestone_m5.py`: fail-closed M5 feature-parity and all-phase closure verdict composer. Its default path consumes one explicit `m5-feature-surface` evidence sidecar plus referenced C++/AMFlow comparison summaries, requires every frozen example class to have a promoted retained golden, coefficient-bearing comparison, unchanged tolerance, and accepted live-path or explicitly reviewed retained-state-exception evidence, and requires every Phase F runtime feature to have accepted evidence. It rejects missing examples, failed comparisons, metadata-only rows, partial references, and open blockers. With `--all-phase-closure-decision`, it also consumes an explicit closure-decision sidecar, validates the committed feature-parity summary against a fresh recomputation, and emits `CLOSED/all-phase` only when both layers pass while preserving M6, M7, and release-readiness non-claims.
...
- `tools/reference-harness/scripts/compare_case_study_numeric_results.py`: first M6 case-study numeric summary producer. It consumes the reviewed case-study readiness summary plus zero or more explicit numeric evidence sidecars, checks each sidecar's family id, digit-threshold profile, failure-code profile, and regression profile against the readiness contract, publishes the per-family numeric profile-label maps, and writes the `case-study-numerics` summary consumed by `qualify_case_study_families.py`. This is still evidence plumbing only: it does not launch the C++ runtime, does not generate case-study numerics, and does not claim `Milestone M6` is passing.
- `tools/reference-harness/scripts/compare_cpp_vs_amflow.py`: first C++ solve-series output ingester. It consumes one `amflow-cli solve-series` JSON result plus either a retained AMFlow `golden-manifest.json` or a raw `j[...] -> Series` rule-list file, matches integrals by family plus indices after default `_amflow` suffix normalization plus optional `--family-alias FROM=TO` mappings, computes per-ε coefficient real/imag digit agreement across the requested ε range while treating omitted coefficients as implicit zeros, and emits a structured pass/fail summary with nonzero exit on missing integrals or tolerance failures. Omitted-coefficient `999/999` agreements are only meaningful when the reference was captured to the requested epsilon order; the phase0 retained `automatic_loop` manifest has no `eps^2` values, so `eps^2` claims must use the eps2-capable manifest at `tools/reference-harness/specs/phase0/automatic_loop.eps2-golden-manifest.json`. This is comparator plumbing only: it does not launch the C++ runtime, does not generate AMFlow goldens, and does not claim phase-0 parity by itself.
- `tools/reference-harness/scripts/extract_amflow_solve_series_state.py`: retained AMFlow solve-series state extractor for cache directories such as `automatic_loop/primary/cache/box1_amflow/1` and `automatic_loop/primary/cache/box2_amflow/1`. It records the actual master basis, `eta` coefficient matrix, inferred singular `eta` locations, AMFlow eta-infinity boundary files, epsilon samples, and Kira target/reduction context into JSON while retaining the state-capture-time note that the direct C++ solver could not yet ingest that physical boundary state. `amflow-cli solve-series` can now load either one such JSON state or the committed automatic-loop state bundle, evaluate the retained leading eta-infinity subsystem-sample boundary coefficients, apply the retained `eta=0` branch-log endpoint coefficient transport through `eps^1` for `<family>[1,0,1,0]` plus the retained selected box endpoint transport through `eps^1` for `<family>[1,1,1,1]` on the retained `box1` and `box2` families, apply retained target reduction after those endpoint corrections exist, and emit comparator-readable results for both family surfaces; this is still partial endpoint/coefficient evidence only, not full singular `eta -> 0` continuation or a phase-0 parity claim.
...
- `tools/reference-harness/scripts/qualify_phase0_packet_set.py`: first retained phase-0 packet-set qualification verdict. It consumes one `qualification_readiness.py` summary plus the packet-set comparison, correct-digit, and failure-code summaries, fail-closes unless their retained packet labels and captured phase-0 ids stay synchronized, fail-closes unless per-benchmark digit-threshold, required-failure-code, and known-regression profiles plus their scaffold profile labels agree across those prerequisite summaries, and writes one blocked/pass verdict over the reviewed phase-0 packet set only while preserving those value and label maps for M6 composition. This remains phase-0-only qualification plumbing: it does not compare retained case-study numerics, does not mark `Milestone M6` complete, and does not widen into M7 sign-off behavior.
- `tools/reference-harness/scripts/qualify_case_study_families.py`: first case-study-family qualification verdict. It consumes one `qualification_case_study_readiness.py` summary plus an optional case-study numeric summary, fail-closes unless their case-study ids stay synchronized, fail-closes unless the numeric summary's per-family rows preserve the observed-digit map and keep non-missing rows aligned with the top-level digit/failure/regression profile-coherence booleans plus the published numeric profile-label maps, and writes one blocked/pass verdict over the reviewed case-study family set only. In the current retained repo state it reports all ten case-study family rows as digit-threshold-met, including the lane103 dedicated diphoton 200-digit verifier evidence, while keeping the phase-0 verdict prerequisite separate. It does not mark `Milestone M6` complete or widen into M7 sign-off behavior.
- `tools/reference-harness/scripts/qualify_milestone_m6.py`: first Milestone M6 verdict composer. It consumes the reviewed phase-0 packet-set verdict plus the reviewed case-study-family verdict, fail-closes unless both subverdicts are passing and no phase-0 runtime-lane blockers remain, and writes one M6-scoped blocked/pass summary while preserving phase-0, case-study, runtime-lane blockers, phase-0 per-benchmark digit/failure-code/regression profiles and profile labels, and case-study-family digit/failure-code/regression profiles and profile labels. This is still qualification plumbing only: it does not launch the runtime, produce new retained captures, mark `Milestone M7`, or claim release readiness.
- `tools/reference-harness/scripts/release_signoff_readiness.py`: first executable M7 helper. It consumes one machine-readable `qualification_readiness.py` summary plus `templates/release-signoff-checklist.json`, audits that the checklist source/docs targets exist inside the repo, preserves the blocked `next_runtime_lane` frontier from the M6 evidence packet, and writes one fail-closed release-readiness summary that keeps `Milestone M6`, feature-parity closure, retained-reference qualification, and final parity sign-off withheld explicitly until accepted packets and reviewed sidecars are supplied. It may now also consume the accepted M5 all-phase qualification packet so the phase-F feature-parity prerequisite can be validated directly, the accepted M6 qualification packet so the milestone-M6 prerequisite can be validated directly, the phase-0 packet-set qualification verdict from `qualify_phase0_packet_set.py` so M7 readiness keeps the retained correct-digit and failure-code blockers visible instead of collapsing them into a generic qualification blocker, the case-study-family qualification verdict from `qualify_case_study_families.py` so the qualified case-study surface and phase-0 prerequisite stay visible, one qualification-corpus sidecar so closed-corpus blockers stay visible, one performance-review sidecar so timing/scope/rebuild blockers stay visible, one diagnostic-review sidecar so typed-failure review blockers stay visible, one docs-completion sidecar so docs-alignment blockers stay visible, and one parity-signoff sidecar so final signoff blockers stay visible in the M7 summary. This remains release-prep plumbing only: it does not run qualification, performance, diagnostic, docs completion, or parity signoff review, and it reaches `release_signoff_ready=true` only when every release prerequisite and review section is already satisfied by explicit sidecars.
- `tools/reference-harness/scripts/review_release_qualification_corpus.py`: first M7 qualification-corpus sidecar producer. It consumes one retained `qualification_readiness.py` summary plus optional phase-0 packet-set and case-study-family qualification verdicts, audits the release-signoff checklist qualification-corpus input/output contract, and writes one blocked `release-qualification-corpus` sidecar that `release_signoff_readiness.py` can consume directly. This remains release-prep plumbing only: it does not rerun qualification numerics, close `Milestone M6` or `Milestone M7`, or claim release readiness.
...
- `tools/reference-harness/scripts/release_status_badge.py`: Shields-compatible M7 release status badge renderer. It consumes the committed release health JSON path, or runs `release_health_summary.py --format json` directly, and emits only the badge label, message, and color for the current release state. This remains status reporting only: it does not rerun AMFlow numerics, create or accept sidecars, or widen release-readiness claims.
...
- `tools/reference-harness/templates/qualification-benchmarks.json` is the first machine-readable
  ...
  current `b61n` / `b63n` / `b64ag` phase-0 blocker map from scratch. Those hints stay aligned
  with the current theory frontier while still anchoring against the recorded predecessor slices
  `b61k` / `b62o` / `b63m` / `b64af`. Ready optional examples may
...
- `tools/reference-harness/scripts/compare_case_study_numeric_results.py` is the first M6
  case-study numeric summary producer. It consumes that readiness summary plus explicit numeric
  evidence sidecars, validates case-study ids and scaffold metadata, preserves per-family numeric
  profile-label maps, reports missing evidence, comparison failures, and correct-digit threshold
  failures per family, and emits the `case-study-numerics` summary consumed by the case-study-
  family qualification verdict. It still does not launch the runtime, produce new retained
  numerics, compare phase-0 evidence, or claim `Milestone M6` is passing.
...
- The first packet-set M6 candidate failure-code audit is
  `tools/reference-harness/scripts/audit_phase0_packet_set_failure_codes.py`. It composes that
  reviewed packet-level audit across the retained `required-set`, `de-d0-pair`, and
  `user-hook-pair` split, requires one unique candidate packet label per root, requires each
  candidate packet root to publish exactly the packet-summary benchmark split for that packet, and
  requires the audited benchmark ids to match the scaffold's full current `reference-captured`
  phase-0 set exactly, but it still does not compare canonical outputs, does not score correct
  digits, does not compare case-study numerics, and does not claim `Milestone M6` is passing.
- The first retained phase-0 packet-set qualification verdict is
  `tools/reference-harness/scripts/qualify_phase0_packet_set.py`. It consumes one
  `qualification_readiness.py` summary plus the packet-set comparison, correct-digit, and
  failure-code summaries, fail-closes unless their retained packet labels and captured phase-0
  ids stay synchronized, fail-closes unless their per-benchmark digit-threshold,
  required-failure-code, and known-regression profiles plus their scaffold profile labels agree,
  and writes one blocked/pass verdict over the reviewed phase-0 packet set only with those value
  and label maps preserved, but it still does not compare
  retained case-study numerics, does not mark `Milestone M6` complete, and does not widen into M7
  release sign-off.
...
- The first executable M7 helper is
  ...
  phase-0 packet-set qualification verdict, the retained case-study-family qualification verdict,
  plus qualification-corpus, performance-review, diagnostic-review, docs-completion, and
  parity-signoff summary sidecars, audits the checklist source/doc paths, and writes one blocked release-readiness
  summary that keeps the
  current `b61n` / `b63n` / `b64ag` phase-0 frontier, phase-0 correct-digit/failure-code
  blockers, qualification-corpus blockers, performance-review blockers, typed-failure
  diagnostic-review blockers, and docs-alignment blockers plus the final parity-signoff blocker path visible while
  `Milestone M6` and the later release-review sections remain open.
- The first qualification-corpus sidecar producer is
  `tools/reference-harness/scripts/review_release_qualification_corpus.py`. It checks the live
  release checklist qualification-corpus section, consumes retained M6 readiness plus optional
  phase-0 and case-study verdicts, emits the blocked `release-qualification-corpus` summary that
  `release_signoff_readiness.py` already validates, and keeps closed benchmark-family coverage
  plus residual qualification blockers visible.
- The first docs-completion sidecar producer is
  ...
  `release-docs-completion` summary that `release_signoff_readiness.py` already validates, and
  keeps explicit non-claims in the sidecar so docs completion cannot be mistaken for broader
  qualification or release sign-off.
- The first performance-review sidecar producer is
  `tools/reference-harness/scripts/review_release_performance.py`. It checks the live release
  checklist and qualification scaffold for the expected performance-review input/output scope,
  emits the `release-performance-review` summary that `release_signoff_readiness.py` already
  validates, and keeps mandatory timing, clean-rebuild performance review, and unstable-run review
  blockers visible instead of treating the sidecar as release-ready timing evidence.
- The first diagnostic-review sidecar producer is
  `tools/reference-harness/scripts/review_release_diagnostic.py`. It checks the live release
  checklist and qualification scaffold for the expected diagnostic-review failure-code and
...
the retained-golden promotion path. All twenty-two helpers expose `--self-check` modes so the repo
can rerun the bootstrap, catalog/scaffold coherence, retained-capture regression scenarios,
scaffold validation, qualification-readiness, case-study-family readiness, case-study numeric
summary production, the retained phase-0 packet-set qualification verdict, the case-study-family
qualification verdict, blocked
release-readiness with case-study verdict, qualification-corpus, performance-review,
diagnostic-review, docs-completion, and parity-signoff sidecar preservation,
qualification-corpus sidecar production, performance-review sidecar production,
diagnostic-review sidecar production, docs-completion sidecar production, parity-signoff sidecar
production, the packet-level and packet-set candidate failure-code audits, and the
...
qualification-readiness, case-study-family readiness, case-study numeric summary production, the
retained phase-0 packet-set qualification verdict, the case-study-family qualification verdict,
blocked release-readiness with
case-study verdict, qualification-corpus, performance-review, diagnostic-review, docs-completion,
and parity-signoff sidecar preservation, qualification-corpus sidecar production,
performance-review sidecar production, diagnostic-review sidecar production, docs-completion
sidecar production, parity-signoff sidecar production, the
