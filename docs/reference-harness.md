...
- `tools/reference-harness/scripts/compare_case_study_numeric_results.py`: first M6 case-study numeric summary producer. It consumes the reviewed case-study readiness summary plus zero or more explicit numeric evidence sidecars, checks each sidecar's family id, digit-threshold profile, failure-code profile, and regression profile against the readiness contract, and writes the `case-study-numerics` summary consumed by `qualify_case_study_families.py`. This is still evidence plumbing only: it does not launch the C++ runtime, does not generate case-study numerics, and does not claim `Milestone M6` is passing.
...
- `tools/reference-harness/scripts/compare_case_study_numeric_results.py` is the first M6
  ...
  evidence sidecars, validates case-study ids and scaffold metadata, reports missing evidence,
  comparison failures, and correct-digit threshold failures per family, and emits the
  `case-study-numerics` summary consumed by the case-study-family qualification verdict. It still
  does not launch the runtime, produce new retained numerics, compare phase-0 evidence, or claim
  `Milestone M6` is passing.
