...
- `tools/reference-harness/scripts/extract_amflow_solve_series_state.py`: retained AMFlow solve-series state extractor for cache directories such as `automatic_loop/primary/cache/box1_amflow/1`. It records the actual master basis, `eta` coefficient matrix, AMFlow eta-infinity boundary files, epsilon samples, and Kira target/reduction context into JSON while explicitly reporting that the current C++ direct solver cannot yet ingest that physical boundary state. This is extraction evidence only, not a generated C++ `ProblemSpec` or parity claim.
