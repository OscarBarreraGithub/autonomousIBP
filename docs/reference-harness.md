...
- `tools/reference-harness/scripts/extract_amflow_solve_series_state.py`: retained AMFlow solve-series state extractor for cache directories such as `automatic_loop/primary/cache/box1_amflow/1`. It records the actual master basis, `eta` coefficient matrix, inferred singular `eta` locations, AMFlow eta-infinity boundary files, epsilon samples, and Kira target/reduction context into JSON while retaining the state-capture-time note that the direct C++ solver could not yet ingest that physical boundary state. `amflow-cli solve-series` can now load this JSON shape, evaluate the retained leading eta-infinity subsystem-sample boundary coefficients, apply the first retained `eta=0` branch-log endpoint coefficient transport for `box1[1,0,1,0]`, apply retained target reduction after that coefficient exists, and emit a top-level `continuation` audit object with `transport_applied = true`, `transport_scope = first-eta-zero-branch-log-endpoint-coefficient-only`, and `full_eta_zero_contour_applied = false`; this is still partial endpoint/coefficient evidence only, not full singular `eta -> 0` continuation or a phase-0 parity claim.
