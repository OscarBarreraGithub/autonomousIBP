...
without adding or changing annotations in existing files. It fails if mypy
diagnostics grow or mypy type coverage drops below the pinned baseline. If mypy
is unavailable, it falls back to the pinned tracked-function annotation ratio.
...
| [`verify_m7_release_tarball_size.py`](../../tools/reference-harness/scripts/verify_m7_release_tarball_size.py) | Build the canonical M7 release evidence tarball via `package_m7_release_evidence.py`, measure its compressed byte size, and fail if it leaves the pinned tolerance window. | `m7-release-evidence-tarball-size` and `m7-release-evidence-tarball-size-self-check`. |
| [`verify_amflow_cli_size.py`](../../tools/reference-harness/scripts/verify_amflow_cli_size.py) | Measure the built Release `amflow-cli` executable before and after `strip --strip-all`, and fail if either byte size leaves the pinned tolerance window. | `amflow-cli-binary-size` and `amflow-cli-binary-size-self-check`. |
...
| [`verify_golden_fixture_totals.py`](../../tools/reference-harness/scripts/verify_golden_fixture_totals.py) | Count committed files under `tools/reference-harness/specs/**/goldens/` and fail if the pinned file-count floor or total-byte window regresses. | `golden-fixture-total-bytes-registry` and `golden-fixture-total-bytes-registry-self-check`. |
...
| [`verify_m5_accepted_packet_counts.py`](../../tools/reference-harness/scripts/verify_m5_accepted_packet_counts.py) | Count accepted M5 packet manifest entries by channel, packet kind, packet mode, and source lane, failing if any committed registry floor drops. | `m5-accepted-packet-count-registry` and `m5-accepted-packet-count-registry-self-check`. |
...
| [`verify_public_header_count.py`](../../tools/reference-harness/scripts/verify_public_header_count.py) | Count public headers under `include/` with `.h` or `.hpp` suffixes, failing if the total or per-extension registry floor drops. | `amflow-public-header-count-registry` and `amflow-public-header-count-registry-self-check`. |
...
| [`verify_include_order_baseline.py`](../../tools/reference-harness/scripts/verify_include_order_baseline.py) | Scan tracked C++ files under `include/`, `lib/`, and `src/`, compare include ordering diagnostics against the pinned baseline, and fail on new regressions. | `include-order-baseline` and `include-order-baseline-self-check`. |
| [`verify_shell_cmake_lint_baseline.py`](../../tools/reference-harness/scripts/verify_shell_cmake_lint_baseline.py) | Discover tracked shell and CMake files, record preferred shell/CMake lint tool availability, and fail if lint or fallback syntax diagnostics exceed the pinned baseline. | `shell-cmake-lint-baseline` and `shell-cmake-lint-baseline-self-check`. |
...
| [`verify_release_doc_file_counts.py`](../../tools/reference-harness/scripts/verify_release_doc_file_counts.py) | Count `docs/release/*.md` files by the pinned live-rerun-X, audit, known-gaps, tools, and others registry categories, failing if any category regresses. | `m7-release-doc-file-counts` and `m7-release-doc-file-counts-self-check`. |
...
python3 tools/reference-harness/scripts/verify_release_doc_file_counts.py --verify
python3 tools/reference-harness/scripts/verify_release_doc_file_counts.py --self-check
