...
- `tools/reference-harness/scripts/release_signoff_readiness.py` is the first executable M7
  ...
  can also consume the retained phase-0 packet-set qualification verdict, audits the checklist
  source/docs targets, and writes one blocked release-readiness summary that keeps the current
  runtime-lane frontier plus phase-0 correct-digit/failure-code blockers visible without claiming
  `Milestone M6` or `Milestone M7` closure.
...
and keeps withheld claims explicit, but it does not run performance review, diagnostic review, or
parity sign-off.
