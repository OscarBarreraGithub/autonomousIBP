...
eta-infinity asymptotic transport coefficient, apply the retained `eta=0` branch-log and
selected box endpoint coefficient transport through explicit `eps^2` zero coefficients for
`<family>[1,0,1,0]` and `<family>[1,1,1,1]`, apply retained family-local target reduction,
and then apply the same selected `eps^2` endpoint surface to the retained reduced top-sector
targets `box1[2,0,1,0]`, `box1[1,2,2,1]`, `box2[1,-1,1,0]`, and `box2[2,1,1,1]`. The
comparator-facing JSON now includes explicit order-2 coefficients for those selected top-sector
masters and targets instead of suppressing output above `eps^1`. The current retained comparison
fails honestly with `matched_integral_count=12`, `compared_coefficient_count=54`, and
`passed_coefficient_count=50` at `--eps-order 2 --digits 40 --tolerance-digits 30`; all eight
listed top-sector order-2 coefficients pass at >=30 digits, while four non-selected lower-sector
order-2 coefficients remain open. Shipping a broader physical parity claim from this result would
still be dishonest: the physical comparison endpoint is the singular `eta -> 0` limit reached
through complex continuation, and the current C++ runtime does not yet perform the full contour or
non-selected endpoint extraction on the retained AMFlow-state path.
...
- `tools/reference-harness/scripts/qualify_phase0_packet_set.py` is the first retained phase-0
  ...
  per-benchmark digit-threshold, required-failure-code, and known-regression profiles plus their
  scaffold profile labels agree across those summaries, and writes one blocked/pass verdict over
  the reviewed phase-0 packet set only while preserving those value and label maps and keeping
  case-study numerics and full `Milestone M6` closure withheld explicitly.
...
- `tools/reference-harness/scripts/qualify_milestone_m6.py` is the first Milestone M6
  ...
  phase-0 per-benchmark digit/failure-code/regression profiles and profile labels, and
  case-study-family digit/failure-code/regression profiles and profile labels in one M6-scoped
  summary without launching runtime work or claiming `Milestone M7`.
...
- `tools/reference-harness/scripts/review_release_diagnostic.py` is the first M7
  diagnostic-review sidecar producer. It audits the release-signoff checklist diagnostic-review
  required inputs/outputs and the qualification scaffold required failure-code plus
  known-regression metadata, then writes one consumer-compatible `release-diagnostic-review`
  summary for `release_signoff_readiness.py` while keeping typed failure-path review and retained
  unstable-run diagnostic evidence explicitly blocked.
