...
std::string SerializePropagationForFingerprint(
    ...
    const bool full_eta_zero_contour_applied) {
  std::ostringstream out;
  ...
  out << "scalar_frobenius_endpoint_patch_applied="
      ...
      << "\n";
  out << "coupled_frobenius_endpoint_matcher_applied="
      << (coupled_frobenius_endpoint_matcher_applied ? "true" : "false")
      << "\n";
  out << "full_eta_zero_contour_applied="
      << (full_eta_zero_contour_applied ? "true" : "false") << "\n";
...
bool AppliesReviewedCoupledFrobeniusRuntimeEndpointMatcher(
    const ComplexContourVector& initial_values,
    const std::vector<ComplexContourNumber>& waypoints,
    const ComplexContourPropagationOptions& options) {
  return initial_values.size() > 1 && waypoints.size() > 2 &&
...
std::vector<ComplexContourMatrix> FitEtaTimesMatrixCoefficientsOnCircle(
    const ComplexContourMatrixEvaluator& matrix_evaluator,
    const std::size_t dimension,
  ...
  for (std::size_t sample_index = 0; sample_index < sample_count;
       ++sample_index) {
    const ComplexContourNumber unit =
        UnitComplexAtRootOfUnity(sample_index, sample_count);
    const ComplexContourNumber eta = sample_radius * unit;
    const ComplexContourMatrix sample =
        SampleEtaTimesMatrix(matrix_evaluator, dimension, eta);
    ComplexContourNumber eta_power{1, 0};
    for (std::size_t power = 0; power < coefficient_count; ++power) {
      const ComplexContourNumber scale =
          ComplexContourNumber{1, 0} /
          (eta_power * ComplexContourFloat(std::to_string(sample_count)));
      coefficients[power] =
          AddScaledMatrix(std::move(coefficients[power]), sample, scale);
      eta_power *= eta;
    }
  }
  return coefficients;
...
struct CoupledFrobeniusEndpointMatch {
  bool success = false;
...
ComplexContourVector EvaluateFrobeniusBasisVector(
    const ComplexContourFrobeniusRecurrence& recurrence,
    const ComplexContourNumber& eta,
    const EtaContourHalfPlane half_plane) {
  ComplexContourVector series = MakeZeroVector(recurrence.dimension);
  ComplexContourNumber eta_power{1, 0};
  for (const ComplexContourVector& coefficient : recurrence.coefficients) {
    if (coefficient.size() != recurrence.dimension) {
      throw std::runtime_error("frobenius-basis-coefficient-dimension-mismatch");
    }
    for (std::size_t row = 0; row < recurrence.dimension; ++row) {
      series[row] += coefficient[row] * eta_power;
    }
    eta_power *= eta;
  }
  const ComplexContourNumber branch_factor =
      ComplexPowerForEndpointBranch(eta, recurrence.indicial_root, half_plane);
  for (ComplexContourNumber& value : series) {
    value *= branch_factor;
  }
  return series;
}
...
std::vector<std::size_t> UnanchoredRowIndices(
    const std::vector<bool>& anchored_rows) {
  std::vector<std::size_t> rows;
  for (std::size_t row = 0; row < anchored_rows.size(); ++row) {
    if (!anchored_rows[row]) {
      rows.push_back(row);
    }
  }
...
ComplexContourMatrix BuildSubmatrix(
  ...
  ComplexContourMatrix submatrix(
      row_indices.size(),
      std::vector<ComplexContourNumber>(column_indices.size(),
                                        ComplexContourNumber{}));
  for (std::size_t row = 0; row < row_indices.size(); ++row) {
    for (std::size_t column = 0; column < column_indices.size(); ++column) {
      submatrix[row][column] =
          matrix[row_indices[row]][column_indices[column]];
    }
  }
  return submatrix;
}

ComplexContourVector BuildAnchoredAmplitudeVector(
    const std::size_t dimension,
    const std::vector<std::size_t>& anchor_row_indices,
    const ComplexContourVector& anchor_amplitudes,
    const std::vector<std::size_t>& free_rows,
    const std::size_t active_free_row) {
  if (anchor_row_indices.size() != anchor_amplitudes.size()) {
    throw std::runtime_error("coupled-frobenius-anchor-amplitude-count");
  }
  ComplexContourVector amplitudes = MakeZeroVector(dimension);
  for (std::size_t index = 0; index < anchor_row_indices.size(); ++index) {
    amplitudes[anchor_row_indices[index]] = anchor_amplitudes[index];
  }
  if (active_free_row < free_rows.size()) {
    amplitudes[free_rows[active_free_row]] = ComplexContourNumber{1, 0};
  }
  return amplitudes;
}

CoupledFrobeniusEndpointMatch MatchTwoFreeCoupledFrobeniusConstants(
    const ComplexContourMatrix& basis_matrix,
    const ComplexContourMatrix& endpoint_basis_matrix,
  ...
  if (free_rows.size() != 2 || anchor_row_indices.empty()) {
    throw std::runtime_error("coupled-frobenius-two-free-row-shape");
  }

  const ComplexContourMatrix anchor_endpoint_matrix =
      BuildSubmatrix(endpoint_basis_matrix,
                     anchor_row_indices,
                     anchor_row_indices);
  const ComplexContourMatrix anchor_free_endpoint_matrix =
      BuildSubmatrix(endpoint_basis_matrix,
                     anchor_row_indices,
                     free_rows);
  const ComplexContourVector particular_anchor_amplitudes =
      SolveComplexLinearSystem(anchor_endpoint_matrix,
                               anchor_endpoint_values,
                               ComplexContourFloat("1e-50"));
  const ComplexContourVector particular_amplitudes =
      BuildAnchoredAmplitudeVector(dimension,
                                   anchor_row_indices,
                                   particular_anchor_amplitudes,
                                   free_rows,
                                   free_rows.size());
  const ComplexContourVector particular_match =
      MatrixVectorProduct(basis_matrix, particular_amplitudes);

  std::vector<ComplexContourVector> free_basis_amplitudes;
  free_basis_amplitudes.reserve(free_rows.size());
  ComplexContourMatrix free_match_matrix(
      free_rows.size(),
      std::vector<ComplexContourNumber>(free_rows.size(),
                                        ComplexContourNumber{}));
  for (std::size_t free_column = 0; free_column < free_rows.size();
       ++free_column) {
    ComplexContourVector compensated_anchor_rhs(
        anchor_row_indices.size(), ComplexContourNumber{});
    for (std::size_t anchor_row = 0; anchor_row < anchor_row_indices.size();
         ++anchor_row) {
      compensated_anchor_rhs[anchor_row] =
          -anchor_free_endpoint_matrix[anchor_row][free_column];
    }
    const ComplexContourVector compensated_anchor_amplitudes =
        SolveComplexLinearSystem(anchor_endpoint_matrix,
                                 compensated_anchor_rhs,
                                 ComplexContourFloat("1e-50"));
    ComplexContourVector free_amplitudes =
        BuildAnchoredAmplitudeVector(dimension,
                                     anchor_row_indices,
                                     compensated_anchor_amplitudes,
                                     free_rows,
                                     free_column);
    const ComplexContourVector free_match =
        MatrixVectorProduct(basis_matrix, free_amplitudes);
    for (std::size_t free_row = 0; free_row < free_rows.size(); ++free_row) {
      free_match_matrix[free_row][free_column] =
          free_match[free_rows[free_row]];
    }
    free_basis_amplitudes.push_back(std::move(free_amplitudes));
  }

  ComplexContourVector rhs(free_rows.size(), ComplexContourNumber{});
  for (std::size_t row = 0; row < free_rows.size(); ++row) {
    rhs[row] = match_values[free_rows[row]] -
               particular_match[free_rows[row]];
  }
  const ComplexContourVector free_constants =
      SolveComplexLinearSystem(free_match_matrix,
                               rhs,
                               ComplexContourFloat("1e-50"));

  ComplexContourVector amplitudes = particular_amplitudes;
  for (std::size_t free_column = 0; free_column < free_rows.size();
       ++free_column) {
    for (std::size_t row = 0; row < dimension; ++row) {
      amplitudes[row] +=
          free_constants[free_column] *
          free_basis_amplitudes[free_column][row];
    }
  }
  ...
  for (std::size_t row = 0; row < free_rows.size(); ++row) {
    match.boundary_condition_residual_abs =
        std::max(match.boundary_condition_residual_abs,
                 ComplexAbs(reconstructed_match[free_rows[row]] -
                            match_values[free_rows[row]]));
  }
  match.basis_residual_abs = match.boundary_condition_residual_abs;
  for (std::size_t anchor_index = 0; anchor_index < anchor_row_indices.size();
       ++anchor_index) {
    ...
    match.anchored_source_residual_abs =
        std::max(match.anchored_source_residual_abs,
                 ComplexAbs(match.endpoint_values[source_row] -
                            anchor_endpoint_values[anchor_index]));
  }
  return match;
}
...
CoupledFrobeniusEndpointMatch MatchCoupledFrobeniusEndpointFromSmallEta(
  ...
  for (std::size_t root_index = 0; root_index < dimension; ++root_index) {
    ...
    if (!recurrence.success) {
      throw std::runtime_error(
          "coupled-frobenius-recurrence-failed-for-root-" +
          std::to_string(root_index) + ": " + recurrence.summary);
    }
    ...
    if (!evaluation.success || !evaluation.endpoint_value_available ||
      ...
      throw std::runtime_error(
          "coupled-frobenius-endpoint-evaluation-failed-for-root-" +
  ...
  CoupledFrobeniusEndpointMatch match;
  if (!anchor_row_indices.empty() && free_rows.size() == 2) {
    match =
        MatchTwoFreeCoupledFrobeniusConstants(basis_matrix,
                                              endpoint_basis_matrix,
                                              match_values,
                                              anchor_row_indices,
                                              anchor_endpoint_values,
                                              free_rows,
    ...
    ComplexContourMatrix constraint_matrix(
        dimension,
        std::vector<ComplexContourNumber>(dimension, ComplexContourNumber{}));
    ...
    if (constraint_row != dimension) {
      throw std::runtime_error(
          "coupled-frobenius-source-anchor-constraint-count");
    }
...
ComputeComplexContourEtaZeroFrobeniusRecurrence(
  ...
  if (!IsFiniteFloat(options.tail_sample_radius) ||
      options.tail_sample_radius < 0) {
    return FrobeniusFailure(
        "invalid-tail-sample-radius",
        "b61n eta=0 Frobenius recurrence requires a finite nonnegative tail "
        "sample radius",
        dimension,
        options);
  }
  if (options.tail_sample_radius > 0 &&
      options.tail_sample_count < 2 * (options.order + 2)) {
    return FrobeniusFailure(
        "insufficient-tail-sample-count",
        "b61n eta=0 Frobenius recurrence requires enough circular tail samples "
        "to resolve the requested local order",
        dimension,
        options);
  }
  ...
  try {
    ...
    recurrence.residue_tolerance = options.residue_tolerance;
    recurrence.tail_fit_tolerance = options.tail_fit_tolerance;
    std::vector<ComplexContourMatrix> eta_times_coefficients =
        options.tail_sample_radius > 0
            ? FitEtaTimesMatrixCoefficientsOnCircle(
                  matrix_evaluator,
                  dimension,
                  options.tail_sample_radius,
                  options.order + 1,
                  options.tail_sample_count)
            : FitEtaTimesMatrixCoefficients(matrix_evaluator,
                                            dimension,
                                            options.residue_probe_eta,
                                            options.order + 1);
    if (options.tail_sample_radius > 0) {
      eta_times_coefficients.front() = indicial.residue_matrix;
    }
    ...
    if (!eta_times_coefficients.empty()) {
      ComplexContourNumber check_eta =
          options.residue_probe_eta /
          ComplexContourFloat(std::to_string(options.order + 2));
      if (options.tail_sample_radius > 0) {
        check_eta = ComplexContourNumber{
            options.tail_sample_radius /
                ComplexContourFloat(std::to_string(options.order + 2)),
            0};
      }
...
ComplexContourPropagationResult PropagateComplexContourVector(
  ...
  try {
    ...
    if (coupled_frobenius_endpoint_matcher_applied) {
      const ComplexContourNumber match_eta = propagation_waypoints.back();
      const CoupledFrobeniusEndpointMatch match =
          MatchCoupledFrobeniusEndpointFromSmallEta(
              matrix_evaluator,
              match_eta,
              refined.values,
      ...
      RequireFiniteVector(match.endpoint_values, "coupled-frobenius-endpoint");
    ...
    result.diagnostics.full_eta_zero_contour_applied =
    ...
    result.diagnostics.summary =
        ...
        (result.diagnostics.full_eta_zero_contour_applied ? std::string("true")
                                                          : std::string("false")) +
        "; "
