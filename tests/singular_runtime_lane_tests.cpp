...
void B61nCoupledFrobeniusEndpointMatcherAnchorsReviewedSourceRowsTest() {
  ...
  const B61nContourNumber drifted_source{5, 7};
  const B61nContourNumber expected_coupled_endpoint{3, 4};
  const B61nContourNumber match_eta{0, B61nContourFloat("-0.25")};
  const B61nContourNumber expected_coupled_match =
      expected_coupled_endpoint - reviewed_source_endpoint *
                                      (B61nContourNumber{0, 0} - match_eta);
  const B61nContourNumber start_eta{0, -1};
  const B61nContourNumber drifted_coupled_start =
      expected_coupled_match -
      drifted_source * (match_eta - start_eta);
  const amflow::ComplexContourVector initial = {
      drifted_source,
      drifted_coupled_start,
  };
  const auto evaluator = [](const B61nContourNumber&) {
    return amflow::ComplexContourMatrix{
        {B61nContourNumber{0, 0}, B61nContourNumber{0, 0}},
  ...
  options.refinement_relative_error_tolerance = B61nContourFloat("1e-34");
  options.matrix_fingerprint =
      "synthetic-b61n-source-anchored-coupled-frobenius-wire-v1";
  options.endpoint_local_model_kind = "regular-taylor-r0";
  options.branch_policy =
      "NegIm lower-half-plane b61n source-anchored endpoint wire test";
  ...
  Expect(result.success,
         "b61n source-anchored coupled Frobenius endpoint wire should succeed: " +
             result.diagnostics.summary);
  Expect(result.diagnostics.coupled_frobenius_endpoint_matcher_applied,
         "b61n source-anchored endpoint wire should use the coupled matcher");
  Expect(result.diagnostics.endpoint_extraction_applied,
         "b61n source-anchored endpoint wire should publish endpoint extraction");
  ExpectB61nContourClose(result.endpoint_values[0],
                         reviewed_source_endpoint,
                         B61nContourFloat("1e-40"),
                         "b61n source-anchored matcher should preserve the reviewed "
                         "source endpoint instead of the propagated source drift");
  ExpectB61nContourClose(result.endpoint_values[1],
                         expected_coupled_endpoint,
                         B61nContourFloat("1e-30"),
                         "b61n source-anchored matcher should recover the coupled "
                         "row from the reviewed source endpoint and coupled-row "
                         "small-eta match");
  ExpectContains(result.diagnostics.summary,
                 "coupled_frobenius_source_anchored=true",
                 "b61n source-anchored matcher should document the anchor mode");
  ExpectContains(result.diagnostics.summary,
                 "coupled_frobenius_source_anchor_rows=[0]",
...
void B61nCoupledFrobeniusEndpointMatcherRetargetsNearZeroMatchPointTest() {
  const std::vector<B61nContourNumber> waypoints = {
      {0, -2},
      {0, -1},
      {0, B61nContourFloat("-0.0625")},
      {0, 0},
  };
  const B61nContourNumber source_endpoint{2, -1};
  const B61nContourNumber first_free_constant{3, 4};
  ...
  options.refinement_relative_error_tolerance = B61nContourFloat("1e-34");
  options.matrix_fingerprint =
      "synthetic-b61n-source-anchored-retargeted-two-free-match-v1";
  options.endpoint_local_model_kind = "regular-taylor-r0";
  options.branch_policy =
      "NegIm lower-half-plane b61n retargeted two-free-constant match test";
  options.coupled_frobenius_anchor_row_indices = {0};
  options.coupled_frobenius_anchor_endpoint_values = {source_endpoint};
  ...
  Expect(result.success,
         "b61n retargeted two-free-constant boundary match should propagate: " +
             result.diagnostics.summary);
  ExpectB61nContourClose(result.endpoint_values[0],
                         source_endpoint,
                         B61nContourFloat("1e-40"),
                         "b61n retargeted match should preserve the reviewed "
                         "source endpoint");
  ExpectB61nContourClose(result.endpoint_values[1],
                         first_free_constant,
                         B61nContourFloat("1e-30"),
                         "b61n retargeted match should recover the first free "
                         "endpoint constant");
  ExpectB61nContourClose(result.endpoint_values[2],
                         second_free_constant,
                         B61nContourFloat("1e-30"),
                         "b61n retargeted match should recover the second free "
                         "endpoint constant");
  ExpectContains(result.diagnostics.summary,
...
void B61nEtaZeroFrobeniusRecurrenceEvaluatesSingleRowEndpointTest() {
  const B61nContourNumber probe_eta{0, B61nContourFloat("-1e-20")};
  const B61nContourFloat residue_tolerance("1e-14");
  const B61nContourNumber c0{5, -2};
  const auto evaluator = [](const B61nContourNumber& eta) {
    return amflow::ComplexContourMatrix{
        {B61nContourNumber{1, 0} / eta + B61nContourNumber{2, 0} +
         B61nContourNumber{3, 0} * eta},
  ...
  Expect(equation.success,
         "b61n single-row Frobenius regression should compute the indicial "
         "probe: " +
             equation.summary);
  Expect(equation.indicial_roots_available && equation.indicial_roots.size() == 1,
         "b61n single-row Frobenius regression should expose the scalar root");
  ExpectB61nContourClose(equation.indicial_roots.front(),
                         {1, 0},
                         B61nContourFloat("1e-14"),
                         "b61n single-row indicial root should match the "
                         "hand-computed residue");

  amflow::ComplexContourFrobeniusRecurrenceOptions options;
  options.selected_root_index = 0;
  options.order = 2;
  options.leading_coefficients = {c0};
  options.residue_probe_eta = probe_eta;
  options.residue_tolerance = residue_tolerance;
  ...
  Expect(recurrence.success,
         "b61n single-row Frobenius recurrence should succeed: " +
             recurrence.summary);
  Expect(recurrence.coefficients.size() == 3,
         "b61n single-row Frobenius recurrence should publish c0 through c2");
  Expect(recurrence.regular_tail_matrices.size() == 2,
         "b61n single-row Frobenius recurrence should extract A0 and A1");
  ExpectB61nContourClose(recurrence.regular_tail_matrices[0][0][0],
                         {2, 0},
                         B61nContourFloat("1e-18"),
                         "b61n single-row Frobenius recurrence should extract "
                         "the constant regular tail");
  ExpectB61nContourClose(recurrence.regular_tail_matrices[1][0][0],
                         {3, 0},
                         B61nContourFloat("1e-18"),
                         "b61n single-row Frobenius recurrence should extract "
                         "the eta regular tail");
  ExpectB61nContourClose(recurrence.coefficients[0][0],
                         c0,
                         B61nContourFloat("1e-18"),
                         "b61n single-row Frobenius c0 should preserve the "
                         "caller seed");
  ExpectB61nContourClose(recurrence.coefficients[1][0],
                         B61nContourNumber{2, 0} * c0,
                         B61nContourFloat("1e-14"),
                         "b61n single-row Frobenius c1 should match the "
                         "hand recurrence");
  ExpectB61nContourClose(recurrence.coefficients[2][0],
                         B61nContourNumber{B61nContourFloat("3.5"), 0} * c0,
                         B61nContourFloat("1e-14"),
                         "b61n single-row Frobenius c2 should match the "
                         "hand recurrence");
  ExpectContains(recurrence.summary,
                 "recurrence-ready-for-endpoint-handler",
                 "b61n Frobenius recurrence should expose the M7 handoff hook");
  ...
  Expect(evaluation.success && evaluation.endpoint_value_available,
         "b61n single-row Frobenius eta=0 evaluation should succeed: " +
             evaluation.summary);
  Expect(evaluation.limit_classification == "vanishing-positive-real-exponent",
         "b61n single-row Frobenius eta=0 evaluation should classify rho=1 as "
         "an exact zero endpoint");
  Expect(evaluation.endpoint_value.size() == 1,
         "b61n single-row Frobenius eta=0 evaluation should publish one value");
  ExpectB61nContourClose(evaluation.endpoint_value.front(),
                         {0, 0},
                         B61nContourFloat("1e-80"),
                         "b61n single-row Frobenius eta=0 evaluation should "
                         "return the exact analytic endpoint");
  ExpectContains(evaluation.summary,
                 "full_eta_zero_contour_applied=false",
                 "b61n Frobenius endpoint evaluation should not claim full M6 "
                 "contour closure");
}
...
int main() {
  try {
    ...
    B61nCoupledFrobeniusEndpointMatcherRetargetsNearZeroMatchPointTest();
