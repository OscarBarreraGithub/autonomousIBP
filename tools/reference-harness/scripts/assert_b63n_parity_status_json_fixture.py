...
def main() -> int:
    ...
    try:
        ...
        print(f"b63n parity status JSON output is not valid JSON: {error}", file=sys.stderr)
        print(completed.stdout, file=sys.stderr)
        return 1
    if not isinstance(actual, dict):
        print("b63n parity status JSON output must be a JSON object", file=sys.stderr)
        return 1
