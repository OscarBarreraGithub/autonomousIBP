...
solName = "sol-2023-jpsi-etac-bfactories-seed";
...
meta = Get[metaPath];
expectedTargets = {
  j[jpsiEtacBFactoriesSeed, 1, 1],
  j[jpsiEtacBFactoriesSeed, 2, 1],
  j[jpsiEtacBFactoriesSeed, 1, 2]
};
...
If[!AssociationQ[meta] ||
    Lookup[meta, "selectedBenchmarkAnchorRef", Missing[]] =!= "2023-jpsi-etac-bfactories" ||
    Lookup[meta, "family", Missing[]] =!= "jpsiEtacBFactoriesSeed" ||
    Lookup[meta, "topology", Missing[]] =!= "one-loop equal-mass charm bubble seed" ||
    Lookup[meta, "kinematicRegion", Missing[]] =!= "Euclidean B-factory short-distance continuation" ||
    Lookup[meta, "numeric", Missing[]] =!= {s -> -112, mc2 -> 1} ||
  ...
  Print["metadata does not match the lane150 J/psi eta_c seed capture contract: ", InputForm[meta]];
  Exit[7];
];
...
If[maxAbsImag > 10^-55,
  Print["verification failed: Euclidean J/psi eta_c seed has an unexpected imaginary component: ", InputForm[maxAbsImag]];
  Exit[5];
];
