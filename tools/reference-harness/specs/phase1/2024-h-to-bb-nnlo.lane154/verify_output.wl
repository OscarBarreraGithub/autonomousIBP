...
If[!AssociationQ[meta] ||
    Lookup[meta, "selectedBenchmarkAnchorRef", Missing[]] =!= "2024-h-to-bb-nnlo" ||
    Lookup[meta, "caseStudyRow", Missing[]] =!= "h-to-bb" ||
    Lookup[meta, "family", Missing[]] =!= "hToBbNnloSeed" ||
    Lookup[meta, "topology", Missing[]] =!= "one-loop equal-mass H-to-bb vertex seed" ||
    Lookup[meta, "kinematicRegion", Missing[]] =!= "Euclidean massive-bottom H-to-bb continuation" ||
    Lookup[meta, "numeric", Missing[]] =!= {mh2 -> -7, mb2 -> 1} ||
  ...
  Print["metadata does not match the lane154 h-to-bb seed capture contract: ", InputForm[meta]];
  Exit[7];
];
...
If[!AllTrue[targetNonzeroCounts, # > 0 &],
  Print["verification failed: at least one expected target lacks a nonzero coefficient after anti-fake threshold: ", InputForm[targetNonzeroCounts]];
  Exit[5];
];
...
If[maxAbsImag > 10^-55,
  Print["verification failed: Euclidean h-to-bb seed has an unexpected imaginary component: ", InputForm[maxAbsImag]];
  Exit[5];
];
