...
If[!AssociationQ[meta] ||
    Lookup[meta, "family", Missing[]] =!= "fivePointOneMassProxyBox" ||
    Lookup[meta, "topology", Missing[]] =!= "one-loop complex-mass box proxy" ||
  ...
  Print["metadata does not match the lane113 five-point proxy capture contract: ", InputForm[meta]];
  Exit[7];
];
If[!finiteNumericQ,
...
If[minPrecision < 35,
  Print["verification failed: minimum finite precision below 35 digits"];
  Exit[5];
];
If[maxAbsImag <= 10^-45,
  Print["verification failed: complex proxy has no nonzero imaginary component"];
  Exit[5];
];
...
Print["sample_coefficients: ", InputForm[Take[coefficients, UpTo[5]]]];
Print["sample_nonzero_coefficients: ", InputForm[Take[nonzeroCoefficients, UpTo[5]]]];
